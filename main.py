...
async def set_bot_commands(command_file):
    ...
    try:
        ...
        result = await Telegram.TELETHON_CLIENT(
            bots.SetBotCommandsRequest(
...
async def check_restart(restart_file):
    try:
        ...
        remove(restart_file)
        await Telegram.TELETHON_CLIENT.edit_message(
...
def start_user_account():
    ...
    user = Telegram.TELETHON_CLIENT.loop.run_until_complete(
        get_me(Telegram.TELETHON_USER_CLIENT)
    )
    first_name = user.first_name
    if not user.premium:
        LOGGER.info(
            f"⛔User Account {first_name} Don't Have Telegram Premium, "
            "2GB Limit Will Be Used For Telegram Uploading."
        )
    else:
...
async def notify_restart(RESTART_NOTIFY_ID):
    try:
        await Telegram.TELETHON_CLIENT.send_message(
            RESTART_NOTIFY_ID, "⚡Bot Started Successfully⚡"
        )
    except Exception as e:
...
if __name__ == "__main__":
    ...
    telethon_bot = Telegram.TELETHON_CLIENT.loop.run_until_complete(
        get_me(Telegram.TELETHON_CLIENT)
    )

    LOGGER.info("🔶Checking For Restart Notification")
    if exists(".restartmsg"):
        Telegram.TELETHON_CLIENT.loop.run_until_complete(
            check_restart(".restartmsg")
        )
    elif Config.RESTART_NOTIFY_ID:
        Telegram.TELETHON_CLIENT.loop.run_until_complete(
            notify_restart(Config.RESTART_NOTIFY_ID)
        )

    # ================= PYROGRAM FIXED START =================
    if Config.USE_PYROGRAM:
        ...
        for attempt in range(5):
            try:
                ...
                LOGGER.info(
                    f'✅Pyrogram Session For @{pyrogram_bot.get_me().username} '
                ...
                LOGGER.warning(
                    f"⏳ Pyrogram time sync issue, retrying in 5s "
                    f"({attempt + 1}/5)"
                )
                time.sleep(5)
        ...
        if not pyrogram_bot:
            LOGGER.error("❌ Pyrogram could not be started after retries")
    ...
    LOGGER.info(f'✅@{telethon_bot.username} Started Successfully!✅')
    LOGGER.info("⚡Bot By  Tdbotdev ⚡")
